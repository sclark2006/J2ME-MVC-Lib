...
public class PersonEdit extends EditView implements ItemStateListener {
    ...
    private void initComponents() {        
        ...
        this.addCommand(this.saveCommand = new Command("Save", Command.SCREEN, 1));
        this.addCommand(this.newItemCommand = new Command("New", Command.SCREEN, 2));
        this.addCommand(this.deleteCommand = new Command("Delete", Command.SCREEN, 3));
        this.addCommand(this.loansCommand = new Command("Loans...", Command.SCREEN, 4));

        this.append(new TextField("Name: *", null, 30, TextField.ANY
    ...
    public void commandAction(Command command, Displayable display) {
        ...
        if (loansCommand.equals(command))
            ((PeopleController) controller).loans().list(Loan.PERSON_ID,
                    controller.getCurrentItem().get(Person.ID), Loan.ID,
                    Collections.ORDER_ASCENDING);
    }

    public void itemStateChanged(Item item) {
        // If is the BirthDate field
        if (item.equals(get(4))) {
            TimeSpan age = TimeSpan.between(new Date(),
                    ((DateField) item).getDate());
            if (age != null)
